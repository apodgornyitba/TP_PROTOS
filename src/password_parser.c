...
enum password_parser_state pass_word_search_handler(struct password_parser *p, uint8_t b);

enum password_parser_state user_read_handler(struct password_parser *pParser, uint8_t b);

enum password_parser_state pass_read_handler(struct password_parser *pParser, uint8_t b);

enum password_parser_state password_parser_feed(struct password_parser *p, uint8_t b) {
    enum password_parser_state ret;
    switch (p->current) {
...
enum password_parser_state user_read_reset(struct password_parser *p, uint8_t b) {
    p->current_index = 0;
    p->last = false;
...
enum password_parser_state pass_read_reset(struct password_parser *p, uint8_t b) {
    if (p->current_index != 0)
        free(p->password);
    free(p->username);
    p->current_index = 0;
